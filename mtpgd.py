...
def one_step(image, grad, step_size):
    return image.data + step_size * torch.sign(grad)


def mtpgd_attack(img, orig_label, net, restart_number, sel_cl_num, eps, step_size, max_epoch):
    img_min = clip_by_tensor(img - 2.0 * eps, -1.0, 1.0)
    img_max = clip_by_tensor(img + 2.0 * eps, -1.0, 1.0)
    
    gt_output = net(img)
    _, b = gt_output.sort(descending=False)
    c = b[0, (1000 - sel_cl_num):1000]
    
    flag = True
    for i in range(restart_number):
        for j in range(sel_cl_num):
            ...
            for k in range(max_epoch):
                ...
                if c[j] == orig_label:
                    ...
                    pert_label, loss = margin_loss(output, c[j])
                if pert_label != orig_label:
                    flag = False
                    break
                loss.backward()
                grad = adv_img.grad.data
                adv_img = one_step(adv_img, grad, step_size)
                adv_img = clip_by_image(adv_img.data, img_min, img_max)
        ...
        if not flag:
            break
    return adv_img.detach(), pert_label, i, j+1, k+1
...
def main():
    ...
    for img_path, img, label in tqdm(data_loader):
        ...
        if (model(img).argmax(1) != label):
            incorrect += 1
            continue
        perturbed_img, final_label, re_num, nd_num, iter_num = mtpgd_attack(img, label, model, \
               opt.restart_number, opt.sel_cl_num, opt.eps, opt.step_size, opt.max_epoch)
        if final_label != label:
            success += 1
            save_image(img_path, img, perturbed_img, opt.output_dir)
        print('Imagename: {}\tlabel_orig: {}\tfinal_label: {}\trestart_number: {}\tnd_number: {}\titer_num: {}'.format(\
            img_path[0].split('/')[-1], label.item(), final_label.item(), re_num, nd_num, iter_num))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, \
       default='/home/guest/liu_hanwen/test_images/mtpgd_vgg16_sign', help='保存图片路径')
    ...
    parser.add_argument('--restart_number', type=int, default=10, help='')
    parser.add_argument('--sel_cl_num', type=int, default=10, help='')
