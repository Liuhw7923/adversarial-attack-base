...
def nim_attack(img, orig_label, net, momentum, restart_number, target, eps, step_size, max_epoch):
    ...
    for i in range(restart_number):
        ...
        for j in range(max_epoch):
            zero_gradients(nes)
            output1 = net(adv_img)
            pert_label = output1.argmax(1)
            if pert_label != orig_label:
                flag = False
                break
            nes = adv_img.data + step_size * momentum * noise
            nes.requires_grad = True
            output2 = net(nes)
            _, loss = margin_loss(output2, orig_label)
            loss.backward()
            grad = nes.grad.data
        ...
        if not flag:
            break
    return adv_img.detach(), pert_label, i, j+1
...
def main():
    ...
    for img_path, img, label in tqdm(data_loader):
        ...
        if (model(img).argmax(1) != label):
            incorrect += 1
            continue
        perturbed_img, final_label, re_num, iter_num = nim_attack(img, label, model, \
                opt.momentum, opt.restart_number, opt.target, opt.eps, opt.step_size, opt.max_epoch)
        if opt.target:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, \
       default='/home/guest/liu_hanwen/test_images/nim_vgg16_sign', help='保存图片路径')
    ...
    parser.add_argument('--momentum', type=float, default=1.0, help='')
    parser.add_argument('--batch_size', type=int, default=1, help='同时处理图像个数，仅支持1')
    parser.add_argument('--target', type=bool, default=False, help='仅支持无目标攻击')
