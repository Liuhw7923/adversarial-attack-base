...
def mim_attack(img, orig_label, net, momentum, restart_number, target, eps, step_size, max_epoch):
...
def main():
    ...
    for img_path, img, label in tqdm(data_loader):
        ...
        if (model(img).argmax(1) != label):
            incorrect += 1
            continue
        perturbed_img, final_label, re_num, iter_num = mim_attack(img, label, model, \
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, \
       default='/home/guest/liu_hanwen/test_images/mim_vgg16_sign', help='保存图片路径')
